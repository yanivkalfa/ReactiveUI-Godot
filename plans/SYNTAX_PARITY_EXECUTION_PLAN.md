...
### T1.4 — `_split_return`: last-top-level-return semantics (Unity parity)  · effort: medium · Status: ⬜
...
### T1.5 — Unknown tags validated; markup parse errors surfaced live  · effort: medium · Status: ⬜
...
### T2.1 — Markup comments  · effort: medium · Status: ⬜
...
### T2.2 — `<Fragment>` named alias  · effort: small · Status: ⬜
**Target.** `<Fragment>...</Fragment>` ≡ `<>...</>` (Unity accepts it case-insensitively, UP:649-680;
mirror that). No attributes allowed except `key` if Unity allows it — check UP:649-680 first, mirror
exactly. Both parsers + vocabulary.json + completion. Fixture + golden.
...
### T2.4 — Text interpolation: Unity semantics + migration warning  · effort: medium · Status: ⬜
...
### T2.6 — Naming checks: PascalCase components, `use_` hook prefix  · effort: small · Status: ⬜
