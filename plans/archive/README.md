...
| `ES_MODULES_EXECUTION_PLAN.md` | ES-modules Layer 2 (family contract: `../ES_MODULES_GENERAL_PLAN.md`) — **EXECUTED**, shipped reactive_ui 0.11.0 / editor 0.9.0 / extensions 0.11.0 (2026-07-18): wrapper keywords deprecated (GUITKX2320 window), plain signature-classified declarations + value exports + full ES import surface, whole tree modernized via `dev/migrate_0_11_0.gd`. Family corpus re-pin deferred until the Unreal leg lands its cases (recorded in MASTER_PLAN §2). **Recorded divergences vs the plan text** (post-audit): (1) the plan contradicted itself on the render alias when the binding decl is a VALUE — §3.2 sketched "no render alias", E-10 locked "render_component() stays the single source of truth" (first exported component still emits `render`); execution follows **E-10** (the locked-decision section wins over an illustration), self-consistent across emitter + export tables + resolver and test-pinned. (2) §3.2's "values grouped before callables" is illustrative — the emitter preserves SOURCE ORDER (the only load-order-sensitive property, relative value order, is preserved). (3) Zero-export files still bind to their first decl (E-10 line 1, "binding model UNCHANGED"); the strict fully-private-file rule stays off, as in 0.10.0. |
