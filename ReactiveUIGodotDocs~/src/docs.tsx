    ...
    pages: [
      {
        ...
        searchContent: 'getting started reactive ui for godot function-style .guitkx components compiler produces sibling .gd class no boilerplate install copy addons/reactive_ui folder enable plugin project settings plugins create a guitkx component setup code returned markup codegen emits static render mount ReactiveRoot.create V.fc ReactiveRootNode setup _ready _exit_tree unmount class_name inferred from declaration no filename convention HelloWorld component var s useState return VBoxContainer Label Hello reactive godot Button Increment onPressed s[1].call s[0] + 1 splitting into files optional hooks styles utils values export import res://addons/reactive_ui extends RefCounted render props children RUIVNode',
    ...
    pages: [
      {
        ...
        searchContent: 'imports exports import { Name } from specifier relative ./ ../ root ~/ extensionless .guitkx implied ES import surface named rename remote as local namespace import * as default import export list export { a b } export default re-exports deferred export declaration reachable cross-file binding class_name first exported multiple declarations mixed component hook util value file-private strict resolution error GUITKX2300 2301 2302 2303 2304 2305 2306 2307 2308 2309 2320 2321 2322 2323 2324 2325 2326 2327 unresolved not exported not declared duplicate unused defined but not imported value-import cycle chain used but unexported boundary module-scoped preamble deprecated wrapper keyword migration codemod migrate_0_11_0.gd idempotent ~/ root guitkx.config.json root key res:// lazy by path component cycles legal eager preload value cycle error no default export alias collision duplicate export default',
    ...
    pages: [
      {
        ...
        title: 'Migrations',
        path: '/migrations',
        keywords: ['migration', 'migrate', 'upgrade', 'codemod', 'breaking', '0.9', '0.10', '0.11'],
        searchContent: 'migrations migrate upgrade breaking release codemod dry-run idempotent 0.8 to 0.9 naming loyalty renames VBox VBoxContainer RichText RichTextLabel onClick onPressed onChange onValueChanged fill anchors_preset migrate_0_9_0.gd MIGRATION-0.9.md 0.9 to 0.10 imports exports strict cross-file resolution export declaration import lines migrate_0_10_0.gd MIGRATION-0.10.md ambient class_name hand-written scripts run in order 0.10 to 0.11 ES modules wrapper keywords component hook module removed signature classification RUIVNode annotation use_ prefix util value export module hoist @class_name namespace import * as default export GUITKX2320 migrate_0_11_0.gd MIGRATION-0.11.md',
    ...
    pages: [
      {
        ...
        keywords: ['files', 'modules', 'export', 'file-private', 'class_name', 'hook', 'styles', 'utils', 'values'],
        searchContent: 'files modules a file is a module ES modules per-file scope export file-private visibility plain declarations signature classification component hook util value binding identity @class_name escape hatch interop first exported declaration file rename module identity directory layout compiler produces sibling .gd class PlayerCard.hooks.guitkx custom hooks reusable state logic use_ prefix Hooks.useState Hooks.useEffect PlayerCard.style.guitkx value exports static var Color colours sizes namespace import import * as PlayerCard.utils.guitkx pure helper formatting functions format_health standalone shared files SharedColors extends RefCounted auto-generated sibling class_name .guitkx.diags.json hot reload',
    ...
    pages: [
      {
        ...
        searchContent: 'asset loading preload load compile-time runtime res:// resource path Texture2D FontFile Theme StyleBox AudioStream PackedScene Resource TextureRect texture Button icon TextureButton texture_normal AudioStreamPlayer theme prop style dict channels icons fonts styleboxes supported file types png jpg webp svg ttf otf woff woff2 fnt tres theme ogg wav mp3 tscn scn .import godot importer',
        element: () => <AssetsPage />,
      },
    ],
  },
    ...
    pages: [
      {
        ...
        searchContent: 'guitkx language reference directives syntax control flow expressions compiles to sibling gdscript .gd class plain declarations signature classification component -> RUIVNode annotation hook use_ prefix util value := export export default export list preamble directives @class_name MyButton override generated gdscript class_name @uss res://ui/theme.tres associate Theme StyleBox resource path function-style components Name() -> RUIVNode typed parameters default props dictionary render props children var s useState return VBoxContainer Label text Button onPressed markup control flow @if @elif @else @for @while @match @case @default bare markup no return wrapper statement-level {expr} markup child gdscript expression RUIVNode array string null attr={expr} attribute value literal plain string onXxx event handler Callable connected to mapped signal # line comment fragment <> </> invisible wrapper structural attributes key ref style event handlers on plus PascalCase signal name onPressed pressed onTextSubmitted text_submitted onFocusEntered focus_entered onFocusExited focus_exited onButtonDown onResized resized onValueChanged value_changed onItemSelected item_selected onTabChanged tab_changed onToggled toggled on_<signal> escape hatch modules hooks Hooks.useState Hooks.useEffect rules gotchas hook calls unconditional top level single root element class_name inferred override else first exported declaration else first declaration @class_name optional rarely needed',
      ...
      },
      {
        ...
        searchContent: 'diagnostics reference diagnostic code guitkx compiler language server severity meaning fix compile time .guitkx files sibling .guitkx.diags.json push_error push_warning godot output parser diagnostics GUITKX0300 unexpected missing token GUITKX0301 unclosed tag GUITKX0302 mismatched closing tag GUITKX0303 missing block unexpected eof GUITKX0304 unclosed brace paren GUITKX0305 unknown @directive GUITKX2506 directive shape error structural semantic diagnostics GUITKX2101 no declaration no markup return GUITKX0103 retired component name differs from file name GUITKX0104 duplicate sibling key GUITKX0106 loop element missing key GUITKX0108 multiple root elements GUITKX2504 invalid module deprecated wrapper GUITKX2505 duplicate declaration GUITKX0026 statement in embedded expression declaration export diagnostics GUITKX2320 deprecated wrapper keyword GUITKX2321 use_ returns RUIVNode GUITKX2322 reserved not emitted GUITKX2323 export non-declaration GUITKX2324 duplicate export GUITKX2325 import alias collision GUITKX2326 no default export GUITKX2327 duplicate export default GUITKX2203 retired language server diagnostics editor extension GUITKX0105 unknown element did you mean suggestion GUITKX0109 unknown attribute on host element classdb runtime hook validation GUITKX0013 hook called conditionally in block positional-slot model hook-order validator RUIConfig.enable_hook_validation RUIDiagnostics push_error component hook module @if @for @while @match @case @default @class_name @extends @use',
