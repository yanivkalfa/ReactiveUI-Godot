    ...
    pages: [
      {
        ...
        searchContent: 'guitkx language reference directives syntax control flow expressions compiles to sibling gdscript .gd class declarations component hook module preamble directives @class_name MyButton override generated gdscript class_name @uss res://ui/theme.tres associate Theme StyleBox resource path function-style components component Name typed parameters default props dictionary render props children var s useState return VBox Label text Button onClick markup control flow @if @elif @else @for @while @match @case @default bare markup no return wrapper statement-level {expr} markup child gdscript expression RUIVNode array string null attr={expr} attribute value literal plain string onXxx event handler Callable connected to mapped signal # line comment fragment <> </> invisible wrapper structural attributes key ref style event handlers camelCase onClick pressed onSubmit text_submitted onFocus focus_entered onBlur focus_exited onPointerDown onResize resized onChange polymorphic text_changed value_changed item_selected tab_changed toggled on_<signal> escape hatch modules hooks Hooks.useState Hooks.useEffect rules gotchas hook calls unconditional top level single root element declaration name matches file @class_name',
