...
### IDE extensions
- **Shared language server — VS Code 0.2.3 + VS 2022 0.2.3** (both extensions bundle the same Node
  server, so these apply to both): **forces tab indentation** when formatting (markup + embedded
  GDScript no longer mix tabs/spaces); **flags unknown host-element attributes** (e.g. a typo'd
  `te`/`xt` on `<Label>`) with a did-you-mean, from the bundled Godot ClassDB data; and reads a
...
- **VS Code only — 0.2.2**: fixes a packaging bug that shipped the VSIX without its
