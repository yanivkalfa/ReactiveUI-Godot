...
### IDE extensions
- VS Code extension bumps to **0.2.2**: fixes a packaging bug that shipped the VSIX without its
  `vscode-languageclient` dependency (so the extension never activated — no formatting / completion
  / hover); adds the `onLanguage:guitkx` activation event, format-on-save defaults for `.guitkx`,
  and a "Restart Language Server" command.
